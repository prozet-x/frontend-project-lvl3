...
const app = () => {
  ...
  const watchedStateStatus = onChange(state.status, (path, newValue) => renderStatus(addingStatusPEl, inputRSSInputEl, state.status, path, newValue)); // Вотчер, следящий за статусом попытки добавления новой RSS-ленты юзером.
  ...
  rssForm.addEventListener('submit', (e) => { // Навесим обработчик для этой формы на событие отправки
    ...
    schema.validate(inputRSSInputEl.value) // Проверим, все ли хорошо ввел юзер. Тут важно, что, если он ничего не ввел вообще, то есть, поле ввода совсем пустое, то ощибку ему покажет сам браузер, так как поле ввода обязательно. Это сделано средствами HTML.
