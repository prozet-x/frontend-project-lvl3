...
import { renderStatus, renderFormAvailability } from './view';
...
const inputRSSInputEl = document.getElementById('newRSSAdress'); // input. Сюда вводим новый адрес RSS-ленты
const addRSSButton = document.getElementById('addRSSButton'); // Кнопка для отправки для добавления RSS-ленты
...
const app = () => {
  const state = { // Это общий стейт приложения.
    ...
    news: new News(), // Тут будут сами новости. По всем новостным лентам.
    statusOfAddingTry: { // Тут - статус попытки добавить новую ленту.
      text: '', // Содержит текст, который будет выведет при попытке добавить новую ленту.
      error: undefined, // Флаг ошибки. Если ввели неверный адрес RSS, то будет истиной. Если верный - ложью.
    },
    formIsAvailable: true, // Указывает, доступна ли форма для взаимндействия, или нет.
  };
  ...
  const watchedStateStatus = onChange(state.statusOfAddingTry, (path, newValue) => renderStatus(addingStatusPEl, inputRSSInputEl, path, newValue)); // Вотчер, следящий за статусом попытки добавления новой RSS-ленты юзером.
  const watchedStateNews = onChange(state.news, () => render(state));
  const watchedStateFormAvailable = onChange(state.formIsAvailable, (_path, isAvailable) => renderFormAvailability(isAvailable));
  ...
  rssForm.addEventListener('submit', (e) => { // Навесим обработчик для этой формы на событие отправки
    ...
    schema.validate(inputRSSInputEl.value) // Проверим, все ли хорошо ввел юзер. Тут важно, что, если он ничего не ввел вообще, то есть, поле ввода совсем пустое, то ошибку ему покажет сам браузер, так как поле ввода обязательно. Это сделано средствами HTML.
      .then((value) => { // Если проблем не вылезло
        watchedStateFeeds.addNewFeed(value, 'TEST NAME'); // Это пока просто для теста
        watchedStateStatus.text = successAddedRSSText; // Статус меняем на 'мол все хорошо'
        watchedStateStatus.error = false; // Ошибки нет
      })
      .catch((err) => { // Если что-то пошло не так
        if (watchedStateStatus.text !== err.message) { // Если текст только что возникшей ошибки не совпадает с текстом ошибки, возникшей при предыдущей попытке добавления, то ...
          watchedStateStatus.text = err.message; // ... скинем в стейт текст ошибки
        }
